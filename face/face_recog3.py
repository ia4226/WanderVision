...
for i, (top, right, bottom, left) in enumerate(face_locations):
    if i in face_names:
        label_position = (left, bottom + 20)
        cv2.putText(resized_image, face_names[i], label_position, cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)

# Resize back to the original size for saving
final_image = cv2.resize(resized_image, (image_bgr.shape[1], image_bgr.shape[0]))

cv2.imwrite("annotated_group_photo.jpg", final_image)
cv2.destroyAllWindows()

print("Final annotated image saved as 'annotated_group_photo.jpg'.")
